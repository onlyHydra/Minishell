...
/*   Updated: 2025/04/16 16:11:05 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *handle_escapes(char *input)
{
    int i = 0;
    int j = 0;
    char *processed = (char *)malloc(sizeof(char) * (ft_strlen(input) + 1));
...
int handle_quotes(char *input, t_parse_state *state)
{
    ...
    if (input[state->i] == '\'' || input[state->i] == '"')
    {
        ...
        add_token(state->tokens, token_value, quote_type);
        state->i = end;
        state->start = state->i;
        return (1);
    }
...
/**
 * Handle operators like pipe and redirections
 * @param input: The input string
 * @param state: Parsing state
 * @return: 1 if handled, 0 otherwise
 */
int handle_operators(char *input, t_parse_state *state)
{
    ...
    if (input[state->i] == '|' || input[state->i] == '>' || input[state->i] == '<')
    {
        if (state->start < state->i && state->in_word)
            process_token(input, state, state->i);
        type = get_token_type(input[state->i]);
        if ((input[state->i] == '>' && input[state->i + 1] == '>') || 
            (input[state->i] == '<' && input[state->i + 1] == '<'))
        {
            type = (input[state->i] == '>') ? REDIRECT_APPEND : HEREDOC;
            add_token(state->tokens, extract_token(input, state->i, state->i + 2), type);
            state->i += 2;
        }
        else
        {
            add_token(state->tokens, extract_token(input, state->i, state->i + 1), type);
            (state->i)++;
        }
        state->start = state->i;
        return (1);
...
int handle_env_vars(char *input, t_parse_state *state)
{
    ...
    if (input[state->i] == '$')
    {
        ...
        if (input[state->i + 1] == '?')
        {
            add_token(state->tokens, extract_token(input, state->i, state->i + 2), EXIT_STATUS);
            end = state->i + 2;
        }
        else
            add_token(state->tokens, extract_token(input, state->i, end), ENV_VAR);
        state->i = end;
        state->start = state->i;
        return (1);
    }
